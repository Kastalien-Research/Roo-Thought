...
LLMUserClientServerLLMUserClientServerInitial request with toolsHuman-in-the-loop reviewExecute tool(s)Continue with tool resultsServer processes result(may continue conversation...)sampling/createMessage(messages + tools)Present request for approvalApprove/modifyForward request with toolsResponse with tool_use(stopReason: "toolUse")Present tool calls for reviewApprove tool callsReturn tool_use responseRun get_weather("Paris")Run get_weather("London")sampling/createMessage(history + tool_results + tools)Present continuationApproveForward with tool resultsFinal text response(stopReason: "endTurn")Present responseApproveReturn final response
