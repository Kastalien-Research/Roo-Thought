...
3. If `execution.taskSupport` is not present or `"forbidden"`, clients **MUST NOT** attempt to invoke the tool as a task. Servers **SHOULD** return a `-32601` (Method not found) error if a client attempts to do so. This is the default behavior.
4. If `execution.taskSupport` is `"optional"`, clients **MAY** invoke the tool as a task or as a normal request.
5. If `execution.taskSupport` is `"required"`, clients **MUST** invoke the tool as a task. Servers **MUST** return a `-32601` (Method not found) error if a client does not attempt to do so.
...
3. From `working`: may move to `input_required`, `completed`, `failed`, or `cancelled`
4. From `input_required`: may move to `working`, `completed`, `failed`, or `cancelled`
5. Tasks with a `completed`, `failed`, or `cancelled` status are in a terminal state and **MUST NOT** transition to any other status
...
Server (Receiver)Client (Requestor)LLMUserServer (Receiver)Client (Requestor)LLMUserLLM initiates requestClient augments with taskClient continues processing other requestswhile task executes in backgroundClient polls for statusServer needs information from clientTask moves to input_requiredClient polls and discovers input_requiredClient opens result streamClient closes result stream and resumes pollingTask continues processing...Task moves back to workingTask completesClient polls and discovers completionClient retrieves final resultsResults retained for ttl period from creationRequest operationtools/call (ttl: 3600000)CreateTaskResult (task-123, status: working)Request other operationOther operation resulttasks/get (task-123)workingtasks/get (task-123)input_requiredtasks/result (task-123)elicitation/create (related-task: task-123)Prompt user for inputProvide informationelicitation response (related-task: task-123)tasks/get (task-123)workingtasks/get (task-123)completedtasks/result (task-123)Result contentProcess result
