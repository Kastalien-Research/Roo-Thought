...
```typescript
// Define a service
class Database extends Context.Tag("Database")<Database, { query: (sql: string) => Effect.Effect<Result> }>() {}
...
const policy = Schedule.exponential("100 millis").pipe(Schedule.intersect(Schedule.recurs(3)))
...
const searchTool = Effect.gen(function* () {
	...
	Effect.catchTag("ParseError", () => Effect.fail({ code: -32602, message: "Invalid params" })),
	Effect.catchTag("DatabaseError", () => Effect.fail({ code: -32603, message: "Internal error" })),
)
```
