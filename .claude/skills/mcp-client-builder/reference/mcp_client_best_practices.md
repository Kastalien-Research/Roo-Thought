...
class AuthorizationManager {
	async checkPermissions(toolName: string, toolAnnotations: ToolAnnotations): Promise<boolean> {
		// Destructive operations require explicit consent
		if (toolAnnotations.destructiveHint === true) {
			return await this.requestUserApproval(`Allow ${toolName}? This operation will modify data.`)
		}
		...
		return await this.requestUserApproval(`Allow ${toolName}? This operation's effects are unknown.`)
	}
}
```
...
async function callToolSafe(client: Client, name: string, args: unknown): Promise<ToolResult> {
	// Get tool schema
...
class DataSharingManager {
	async beforeSendingData(data: unknown, serverInfo: ServerInfo): Promise<void> {
		...
		const consent = await this.requestConsent(`Share data with ${serverInfo.name}?`, this.summarizeData(data))
...
```typescript
async function handleSamplingRequest(request: CreateMessageRequest): Promise<CreateMessageResult> {
...
function truncateResponse(data: unknown, config: TruncationConfig): unknown {
...
function removeLowSignalFields(data: unknown): unknown {
	...
	const lowSignalKeys = ["id", "createdAt", "updatedAt", "metadata", "timestamp", "version", "_links", "_embedded"]
...
const result = await executeWithRetry(() => client.callTool("flaky_api", {}), { maxRetries: 3, baseDelay: 1000 })
```
