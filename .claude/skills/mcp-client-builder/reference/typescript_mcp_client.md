...
const transport = new SSEClientTransport(new URL("https://legacy-server.example.com/sse"))
```
...
// Track roots
const roots = [{ uri: "file:///home/user/project", name: "My Project" }]
...
```typescript
async function callToolWithRetry<T>(fn: () => Promise<T>, maxRetries = 3, baseDelay = 1000): Promise<T> {
...
class MCPHost {
	...
	async connectToServer(serverId: string, config: ServerConfig): Promise<void> {
		const client = new Client(
			{
				name: "my-host",
				version: "1.0.0",
			...
			const toolResults = await Promise.all(
				toolUseBlocks.map(async (toolUse) => {
					...
					return {
						...
						content: result.content.map((c) => (c.type === "text" ? c.text : JSON.stringify(c))).join("\n"),
					}
				}),
			)
