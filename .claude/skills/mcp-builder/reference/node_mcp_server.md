...
This document provides Node/TypeScript-specific best practices and examples for implementing MCP servers using the MCP TypeScript SDK, compliant with the **2025-11-25** specification. It covers project structure, server setup, tool registration patterns, input validation with Zod, error handling, and complete working examples.
...
**Common formats**: snake_case (e.g., "search_users", "create_project") or camelCase (e.g., "searchUsers").
...
```typescript
{
  name: string;           // Unique identifier (required, 1-128 chars)
...
// In tool handler - return Tool Execution Error
;async (params: SearchInput) => {
	try {
		// ... implementation
	} catch (error) {
		return {
			isError: true, // Mark as Tool Execution Error
			content: [
				{
					type: "text",
					text: handleApiError(error),
...
The TypeScript SDK supports different transport mechanisms. The 2025-11-25 spec introduces **Streamable HTTP** as the primary HTTP transport.
...
- **Streamable HTTP** (2025-11-25): Web services, remote access, multiple simultaneous clients
...
- **Legacy SSE** (deprecated): Server-push notifications (use Streamable HTTP instead)
