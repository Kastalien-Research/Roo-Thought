...
async function scrapeTechnology(techName: string): Promise<ScrapedPage[]> {
	...
	for (const url of docUrls.slice(0, 10)) {
		...
		if (content) {
			const filename = new URL(url).pathname.replace(/\//g, "-").replace(/^-|-$/g, "") || "index"
...
async function main() {
	...
	const successPages = Object.values(allResults)
		.flat()
...
function main() {
	...
	// Write results
	writeFileSync(".docs-research/verification-results.json", JSON.stringify(results, null, 2))
